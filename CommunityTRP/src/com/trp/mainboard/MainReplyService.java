...
public class MainReplyService {
	Scanner sc = new Scanner(System.in);
	
	// 댓글 조회
	public void getReplyList(int boardNum) {
		List<MainReply> list = MainReplyDAO.getInstance().getReplyList(boardNum);
		System.out.println("===============================");
	...
	public void writeReply(int boardNum) {
		...
		if (MemberService.memberInfo == null) {
			...
			int result = MainReplyDAO.getInstance().writeReply(reply);
	...
	public void updateReply(int boardNum) {
		...
		reply = MainReplyDAO.getInstance().getReply(replyNum);
		if (MemberService.memberInfo == null) {
			System.out.println("먼저 로그인을 해 주세요.");
			...
			int result = MainReplyDAO.getInstance().updateReply(reply);
			if (result > 0) {
	...
	public void deleteReply(int boardNum) {
		...
		if (MemberService.memberInfo == null) {
			...
			int result = MainReplyDAO.getInstance().deleteReply(reply.getReplyNumber());
			if (result > 0) {
